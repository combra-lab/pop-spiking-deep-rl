...
class SpikingActorNet:
    ...
    def __init__(self,
        ...
        for num in range(len(weights)):
            param_dict, new_vth, new_sf = pytorch_trained_snn_param_2_loihi_snn_param(
                weights[num], bias[num], vth
            )
    ...
    def set_network_bias_layer(self):
        ...
        """
        assert len(self.core_list[-1]) == len(self.loihi_snn_dimension) - 1
        neuron_prototype = nx.CompartmentPrototype(
            compartmentCurrentDecay=2 ** 12
        )
    ...
    def set_online_bias_encoding(self):
        ...
        for core in self.core_list[-1]:
            ...
            pseudo_single.logicalCoreId = core
            pseudo_neurons.addCompartments(pseudo_single)
        conn_w = np.eye(len(self.core_list[-1])) * 120
        conn_mask = np.int_(np.eye(len(self.core_list[-1])))
    ...
    def set_network_connections(self):
        ...
        for layer in range(len(self.loihi_snn_dimension) - 1):
            ...
            bias_pos_conn_w = np.zeros((self.loihi_snn_dimension[layer + 1],
                                        len(self.loihi_snn_dimension)-1))
            bias_pos_conn_mask = np.int_(np.zeros((self.loihi_snn_dimension[layer + 1],
                                                   len(self.loihi_snn_dimension)-1)))
            bias_pos_conn_w[:, layer] = self.loihi_bias['pos_b'][layer][:, 0]
            bias_pos_conn_mask[:, layer] = self.loihi_bias['pos_b_mask'][layer][:, 0]
            ...
            bias_neg_conn_mask = np.int_(np.zeros((self.loihi_snn_dimension[layer + 1],
                                                   len(self.loihi_snn_dimension)-1)))
            bias_neg_conn_w[:, layer] = self.loihi_bias['neg_b'][layer][:, 0]
            bias_neg_conn_mask[:, layer] = self.loihi_bias['neg_b_mask'][layer][:, 0]
            self.network_bias_layer.connect(
                post_neuron,
                prototype=nx.ConnectionPrototype(
                    numWeightBits=8,
    ...
    def setup_snn(self,
        ...
        encoder_channel = board.createChannel(b'encodeinput', "int", encode_pack_num)
        encoder_channel.connect(None, encoder_snip)
        decoder_channel = board.createChannel(b'decodeoutput', "int", decode_pack_num)
